...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Abeychain peer connected enter loop", "name", p.Name())
	for {
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, abeyVersion, p.RemoteAddr().String(), pc)
	pm.fdownloader.RegisterLightPeer(p.id, abeyVersion, pc)
}
